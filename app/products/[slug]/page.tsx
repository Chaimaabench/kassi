...
export default function ProductDetailsPage() {
    ...
    return (
      <main className="min-h-screen bg-cream px-6 py-16">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-10 text-charcoal/60">{t.product.loading}</p>
        </div>
      </main>
    );
  }
      ...
      <main className="min-h-screen bg-cream px-6 py-16">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mt-12 rounded-[2rem] border border-charcoal/10 bg-white p-10">
            <h1 className="font-serif text-4xl text-charcoal">{t.product.notFoundTitle}</h1>
            <p className="mt-4 text-charcoal/60">{t.product.notFoundText}</p>
            <Link
  ...
  const productName = locale === 'fr' ? product.nameFr ?? product.name : product.name;
  const productDescription = locale === 'fr' ? product.descriptionFr ?? product.description : product.description;
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:gap-16">
          <section>
            <div className="relative overflow-hidden rounded-[2rem] bg-white">
              <div className="relative aspect-[4/5]">
                <Image src={selectedImage} alt={productName} fill className="object-cover" />
              </div>
              {discountPercent && (
              ...
              {product.tag && (
                <div className="absolute left-6 top-6 rounded-full bg-white px-4 py-2 text-sm font-medium text-charcoal">
                  {getTagLabel(product.tag, locale)}
                </div>
              )}
            </div>
                ...
                {product.images.map((image, index) => (
                  ...
                  >
                    <div className="relative aspect-square">
                      <Image src={image} alt={`${productName} thumbnail ${index + 1}`} fill className="object-cover" />
                    </div>
                  </button>
                ))}
              </div>
          ...
          <section className="lg:sticky lg:top-8 lg:self-start">
            <div className="rounded-[2rem] border border-charcoal/10 bg-white p-8 shadow-sm">
              <p className="text-sm uppercase tracking-[0.18em] text-charcoal/45">{getCategoryLabel(product.categoryId, locale)}</p>
              <h1 className="mt-3 font-serif text-4xl text-charcoal">{productName}</h1>
              <p className="mt-5 text-lg leading-8 text-charcoal/70">{productDescription}</p>
              ...
              <div className="mt-8 rounded-2xl bg-sage/10 p-5 text-sm text-charcoal/75">
                {t.product.freeShipping} {FREE_SHIPPING_THRESHOLD} MAD. {t.product.cod}
              </div>
